...
import { CleverDeleteTeacherUseCase, IAdministratorRepository, ITeacherRepository } from './CleverDeleteTeacherUseCase'

const VALID_TEACHER_ID = '10dc3c1a-ddda-46f1-90b1-8f982dcd54f7'
const INVALID_TEACHER_ID = 'district-id-00001'

describe('Test CleverDeleteTeacherUseCase', () => {
  test('Test CleverDeleteTeacherUseCase - success', async () => {
    const teacherRepository: ITeacherRepository = {
      deactivateTeacher: jest.fn(async function (): Promise<Errorable<{ message: string }, E<'UnknownRuntimeError'>>> {
        return {
          hasError: false,
          error: null,
          value: {
    ...
    const usecase = new CleverDeleteTeacherUseCase(teacherRepository, administratorRepository)
    ...
    const deleteTeacherSpy = teacherRepository.deactivateTeacher as jest.Mock
