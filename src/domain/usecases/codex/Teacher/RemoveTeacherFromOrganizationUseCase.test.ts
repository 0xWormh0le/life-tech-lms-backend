...
import { RemoveTeacherFromOrganizationUseCase, ITeacherRepository, IOrganizationRepository } from './RemoveTeacherFromOrganizationUseCase'
...
describe('Test RemoveTeacherFromOrganizationUseCase', () => {
  test('Test RemoveTeacherFromOrganizationUseCase - success', async () => {
    const teacherRepository: ITeacherRepository = {
      removeTeacherFromOrganization: jest.fn(async function (): Promise<Errorable<void, E<'UnknownRuntimeError'>>> {
        return {
          hasError: false,
          error: null,
          value: undefined,
    ...
    const usecase = new RemoveTeacherFromOrganizationUseCase(teacherRepository, organizationRepository)
    ...
    const removeTeacherFromOrganizationSpy = teacherRepository.removeTeacherFromOrganization as jest.Mock

    expect(removeTeacherFromOrganizationSpy.mock.calls).toEqual([[VALID_ORGANIZATION_ID, VALID_TEACHER_ID]])
  ...
  test('User role is not internal operator or administrator - permission denied', async () => {
    ...
    expect(result.error?.message).toEqual('The token does not have permission to remove the specified teacher from organization.')
    expect(result.value).toEqual(null)
  })
  ...
  test('When provided invalid organizationId returns invalid error', async () => {
    ...
    expect(removeTeacherFromOrganizationSpy.mock.calls).toEqual([])
  ...
  test('When provided invalid teacherId returns invalid error', async () => {
    ...
    const deletTeacherOrganizationSpy = teacherRepository.removeTeacherFromOrganization as jest.Mock

    expect(deletTeacherOrganizationSpy.mock.calls).toEqual([])
