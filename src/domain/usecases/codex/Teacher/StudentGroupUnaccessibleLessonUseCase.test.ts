...
import {
  CreateUnaccessibleLessonUseCase,
  ITeacherRepository,
  ILessonRepository,
  IStudentGroupRepository,
  IAdministratorRepository,
  IUnaccessibleLessonRepository,
} from './StudentGroupUnaccessibleLessonUseCase'
...
describe('Test CreateUnaccessibleLessonUseCase', () => {
  test('Test CreateUnaccessibleLessonUseCase - success', async () => {
    const unaccessibleLessonRepository: IUnaccessibleLessonRepository = {
      createUnaccessibleLesson: jest.fn(async function (): Promise<Errorable<void, E<'UnknownRuntimeError'>>> {
        return {
          hasError: false,
          error: null,
          value: undefined,
    ...
    const studentGroupRepository: IStudentGroupRepository = {
      ...
      getStudentGroupById: jest.fn(async function (studentGroupId: string): Promise<Errorable<StudentGroup, E<'UnknownRuntimeError'>>> {
        return {
          ...
          error: null,
          value: {
            id: '916c49e8-fe79-4577-8558-c06d6df079b3',
            name: 'Fl Group',

            studentGroupLmsId: '1',
            createdUserId: '1f9a5310-328c-415d-b42b-637766ac4b32',
            updatedUserId: '1f9a5310-328c-415d-b42b-637766ac4b32',
            createdDate: 'test_createdDate_1',
            updatedDate: 'test_createdDate_1',
            organizationId: '5d2a80cc-7bd6-443b-8296-a0e8bab8261a',
            grade: 'grade',
          },
        }
      }),
    ...
    const usecase = new CreateUnaccessibleLessonUseCase(
    ...
    const result = await usecase.run(
      ...
      VALID_LESSON_IDS,
      'package-codeillusion-basic',
    )

    expect(result.hasError).toEqual(false)

    const createUnaccessibleLessonSpy = unaccessibleLessonRepository.createUnaccessibleLesson as jest.Mock

    expect(createUnaccessibleLessonSpy.mock.calls).toEqual([[VALID_STUDENT_GROUP_ID, VALID_LESSON_IDS, 'requested-user-id', 'package-codeillusion-basic']])
  ...
  test('User role is not internal operator or administrator or teacher - permission denied', async () => {
    ...
    expect(result.error?.message).toEqual('The user does not have permission to restrict lesson for access to provided student group')
    expect(result.value).toEqual(null)
  })
  ...
  test('When provided invalid studentGroupId returns invalid error', async () => {
    ...
    expect(createUnaccessibleLessonSpy.mock.calls).toEqual([])
  ...
  test('When fail to get student group', async () => {
    const unaccessibleLessonRepository: IUnaccessibleLessonRepository = {
      createUnaccessibleLesson: jest.fn(async function (): Promise<Errorable<void, E<'UnknownRuntimeError'>>> {
        return {
          hasError: false,
