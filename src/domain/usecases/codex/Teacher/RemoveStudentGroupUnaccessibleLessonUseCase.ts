...
export class RemoveUnaccessibleLessonUseCase {
  constructor(
  ...
  > {
    ...
    if (!ALLOWED_ROLES.includes(user.role)) {
      return {
        ...
        error: {
          ...
          message:
            'The user does not have permission to remove restrict lesson access from provided student group',
        },
        value: null,
      }
    }
    ...
    const unaccessibleLessonResult =
      await this.unaccessibleLessonRepository.removeUnaccessibleLesson(
        studentGroupId,
        lessonIds,
      )
    ...
    if (unaccessibleLessonResult.hasError) {
      return {
        ...
        error: wrapError(
          ...
          `failed to remove restrict lesson access for student group`,
        ),
        value: null,
      }
    }
