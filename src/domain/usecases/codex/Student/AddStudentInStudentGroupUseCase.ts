...
export class AddStudentInStudentGroupUseCase {
  constructor(
  ...
  > {
    ...
    if (!ALLOWED_ROLES.includes(user.role)) {
      return {
        ...
        error: {
          ...
          message: 'The user does not have permission to add the student.',
        },
        value: null,
      }
    }
    ...
    const studentRepositoryResult =
      await this.studentRepository.addStudentInStudentGroup(
        user.id,
        studentGroupId,
        studentId,
      )
      ...
      switch (studentRepositoryResult.error.type) {
        case 'StudentAlreadyExists': {
          return {
            ...
            error: wrapError(
              ...
              `Student already exists in same student group.`,
            ),
            value: null,
          }
        }
        case 'StudentNotFoundError': {
          return {
            ...
            error: wrapError(
              studentRepositoryResult.error,
              `The specified student group not found for studentId: ${studentId}.`,
            ),
            value: null,
          }
        }
        default: {
          return {
            ...
            error: wrapError(
              studentRepositoryResult.error,
              `Failed to add student in student-group`,
            ),
            value: null,
          }
        }
