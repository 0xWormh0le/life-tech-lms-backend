import GetLessonQuizByLessonStepIdUseCase, { LessonQuizRepository } from './GetLessonQuizByLessonStepIdUseCase'
...
describe('GetLessonQuizByLessonStepIdUseCase', () => {
  describe('Authorization', () => {
    ...
    `(
      ...
      async ({ userRole, expectAuthorizationError }: { userRole: UserRole; expectAuthorizationError: boolean }) => {
        ...
        const useCase = new GetLessonQuizByLessonStepIdUseCase(lessonQuizRepository)
  ...
  describe('run', () => {
    ...
    `(`hasRepositoryError: $hasRepositoryError, expectUnknownError: $expectUnknownError`, async ({ hasRepositoryError, expectUnknownError }) => {
      ...
      if (hasRepositoryError) {
        lessonQuizRepository.findByLessonStepId = jest.fn(async (_id: string): Promise<Errorable<LessonQuiz, E<'UnknownRuntimeError'>>> => {
          return {
            hasError: true,
            error: {
  ...
  const createSuccessMockLessonQuizRepository = () => {
    const repo: LessonQuizRepository = {
      findByLessonStepId: async (_id: string): Promise<Errorable<LessonQuiz, E<'UnknownRuntimeError'>>> => {
        return {
          hasError: false,
          error: null,
          value: {
    ...
    return {
      findByLessonStepId: jest.fn((id: string) => repo.findByLessonStepId(id)),
    }
  }
})
