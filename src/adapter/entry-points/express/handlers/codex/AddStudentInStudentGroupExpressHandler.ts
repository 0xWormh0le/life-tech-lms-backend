...
import { AddStudentInStudentGroupUseCase } from '../../../../../domain/usecases/codex/Student/AddStudentInStudentGroupUseCase'

type Response =
  | Paths.PostStudentInStudentGroup.Responses.$200
  | Paths.PostStudentInStudentGroup.Responses.$400
  | Paths.PostStudentInStudentGroup.Responses.$401
  | Paths.PostStudentInStudentGroup.Responses.$403
  | Paths.PostStudentInStudentGroup.Responses.$404
  | Paths.PostStudentInStudentGroup.Responses.$409
  | Paths.PostStudentInStudentGroup.Responses.$500

export class AddStudentInStudentGroupExpressHandler {
  constructor(private appDataSource: DataSource) {}

  handler: HandlerWithToken<
    Paths.PostStudentInStudentGroup.PathParameters,
    undefined,
    undefined,
    Response
  > = async (params, token) => {
    ...
    const createStudentInStudentGroup = new AddStudentInStudentGroupUseCase(
      studentRepository,
    ...
    if (getUserResult.hasError) {
      const response500: Paths.PostStudentInStudentGroup.Responses.$500 = {
    ...
    const studentResult = await createStudentInStudentGroup.run(
      getUserResult.value,
    ...
    if (studentResult.hasError) {
      switch (studentResult.error.type) {
        ...
        case 'PermissionDenied': {
          const response403: Paths.PostStudentInStudentGroup.Responses.$403 = {
            error: studentResult.error.message,
          }
        ...
        case 'StudentAlreadyExists': {
          const response409: Paths.PostStudentInStudentGroup.Responses.$409 = {
            error: studentResult.error.message,
          }
    ...
    const response200: Paths.PostStudentInStudentGroup.Responses.$200 = {
      message: 'ok',
    }
