...
describe('CodexV2 / LessonHint / UserAccessScenario', () => {
  const userTokens = {
    internalOperator: '',
    administrator: '',
    teacher1: '',
    ...
    `(
      ...
      async ({ userToken, expectUnauthorizedError }) => {
        ...
        if (expectUnauthorizedError) {
          expect(res.data.data.lessonHints.__typename).toEqual(
            'ErrorPermissionDenied',
          )
          expect(res.data.data.lessonHints.errorCode).toEqual(
            'PERMISSION_DENIED',
          )
        } else {
          expect(res.data.data.lessonHints.__typename).toEqual('LessonHints')
          expect(res.data.data.lessonHints.items).toBeDefined()
