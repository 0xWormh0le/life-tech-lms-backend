...
describe('CodexV2 / CurriculumBrand / UserAccessScenario', () => {
  const userTokens = {
    internalOperator: '',
    administrator: '',
    teacher1: '',
    ...
    `(
      ...
      async ({ userToken, expectUnauthorizedError }) => {
        ...
        const res = await axios.post<
          GraphQLResponse<'curriculumBrands', { items: [] }>
        ...
        if (expectUnauthorizedError) {
          ...
          expect(res.data.data.curriculumBrands.errorCode).toEqual(
            'PERMISSION_DENIED',
          )
        } else {
          expect(res.data.data.curriculumBrands.__typename).toEqual(
          ...
          expect(res.data.data.curriculumBrands.items).toBeDefined()
