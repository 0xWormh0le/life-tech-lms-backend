...
describe('CodexV2 / CsePackageUnitDefinition / UserAccessScenario', () => {
  const userTokens = {
    internalOperator: '',
    administrator: '',
    teacher1: '',
  ...
  describe('get CsePackageUnitDefinition', () => {
    test.each`
    ...
    `(
      ...
      async ({ userToken, expectUnauthorizedError }) => {
        ...
        const res = await axios.post<
          GraphQLResponse<'csePackageUnitDefinitions', { items: [] }>
        ...
        if (expectUnauthorizedError) {
          ...
          expect(res.data.data.csePackageUnitDefinitions.errorCode).toEqual(
            'PERMISSION_DENIED',
          )
        } else {
          expect(res.data.data.csePackageUnitDefinitions.__typename).toEqual(
          ...
          expect(res.data.data.csePackageUnitDefinitions.items).toBeDefined()
          expect(
            res.data.data.csePackageUnitDefinitions.errorCode,
          ).toBeUndefined()
        }
      },
    )
