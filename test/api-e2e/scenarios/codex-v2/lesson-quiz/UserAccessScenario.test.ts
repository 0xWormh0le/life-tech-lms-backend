...
describe('CodexV2 / LessonQuiz / UserAccessScenario', () => {
  const userTokens = {
    internalOperator: '',
    administrator: '',
    teacher1: '',
    ...
    `(
      ...
      async ({ userToken, expectUnauthorizedError }) => {
        ...
        if (expectUnauthorizedError) {
          expect(res.data.data.lessonQuizzes.__typename).toEqual(
            'ErrorPermissionDenied',
          )
          expect(res.data.data.lessonQuizzes.errorCode).toEqual(
            'PERMISSION_DENIED',
          )
        } else {
          expect(res.data.data.lessonQuizzes.__typename).toEqual(
          ...
          expect(res.data.data.lessonQuizzes.items).toBeDefined()
